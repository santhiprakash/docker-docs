...
The output above also shows the `/usr/bin/docker` Docker CLI profile is
running in `complain` mode. This means AppArmor only logs to `dmesg`
...
Advanced users and package managers can find a profile for the `/usr/bin/docker`
Docker CLI underneath
[contrib/apparmor](https://github.com/moby/moby/tree/master/contrib/apparmor)
